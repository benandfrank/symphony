...
| Tracker / DynamicTool | `create_comment/2` returned `:ok` instead of `{:ok, comment_id}`, making it impossible to chain create → update through the Tracker boundary. `DynamicTool` had no unified tool for workpad comment updates, forcing agents to fall back to tracker-specific raw API tools. | Changed `create_comment/2` callback and all adapters (Linear, ClickUp, Memory) to return `{:ok, comment_id}`. Added `tracker_update_comment` shared dynamic tool for both Linear and ClickUp that delegates to `Tracker.update_comment/2`. API contract complete; agent-side usability tracked separately in Open table. | — | 2026-03-12 |
| DynamicTool / Orchestrator | `tracker_update_comment` tool was advertised but agents had no reliable way to obtain the `comment_id` — the orchestrator did not inject it into the agent prompt context. | `AgentRunner` now creates the workpad comment via `Tracker.create_comment/2` after the `before_run` hook and injects the returned `comment_id` as `workpad_comment_id` into the Liquid prompt context. The `WORKFLOW.md` template exposes it in the Issue context block. Graceful degradation: if `create_comment` fails, the run continues with `workpad_comment_id = nil`. SPEC §5.4 updated. | — | 2026-03-13 |
