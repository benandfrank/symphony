...
|------|---------|--------------|--------|-------|
| DynamicTool / Orchestrator | `tracker_update_comment` tool is advertised but agents have no reliable way to obtain the `comment_id` — the orchestrator does not yet inject it into the agent prompt context. Full agent-side usability is blocked until the create→prompt wiring is implemented. | PR #3 review | — | 2026-03-13 |
